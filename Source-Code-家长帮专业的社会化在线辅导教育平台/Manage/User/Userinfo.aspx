...
<tr>
	...
	<td colspan="3">
		...
		<a href="Addon/UserLogin.aspx?uname=<%=HttpUtility.UrlEncode(mu.UserName)+"&upwd="+mu.UserPwd %>" class="margin_l5" target="_blank"><i class="zi zi_externalLinkalt" zico="外部链接"></i> 以此会员登录</a>
		<a href="UserModify.aspx?UserID=<%=mu.UserID %>" class="btn btn-outline-info" title="修改信息"><i class="zi zi_pencilalt" zico="笔黑"></i>修改</a>
		<%if (!pmu.IsNull)
            {%><a href="Userinfo.aspx?id=<%:mu.ParentUserID %>" class="btn btn-outline-info" title="点击查看"><i class="zi zi_user" zico="用户标志"></i> 推荐人：<%=pmu.UserName + "(" + pmu.UserID + ")" %></a><%}
    else
...
<tr>
	...
	<td colspan="3">
		<div class="input-group mb-3">
		  <div class="input-group-prepend">
			<span class="input-group-text"><i class="zi zi_tmWeixin text-success"></i></span>
		  </div>
		...
		<div class="input-group mb-3">
		  <div class="input-group-prepend">
			<span class="input-group-text"><i class="zi zi_tmWeibo text-danger"></i></span>
		  </div>
		...
		<div class="input-group mb-3">
		  <div class="input-group-prepend">
			<span class="input-group-text"><i class="zi zi_tmQq text-info"></i></span>
		  </div>
						...
						<asp:Repeater runat="server" ID="UPro_RPT">
							<ItemTemplate>
								<div class="btn btn-default up_proitem">
									...
									<a href="javascript:;" style="z-index: 10;" class="pull-right" onclick="upro.del(this,'<%#Eval("ID") %>');"><i class="zi zi_trashalt" zico="垃圾箱竖条"></i></a>
								</div>
							</ItemTemplate>
						</asp:Repeater>
					</div>
...
<a href="UserModify.aspx?UserID=<%=UserID %>" class="btn btn-outline-info" title="修改信息"><i class="zi zi_pencilalt" zico="笔黑"></i> 修改资料</a>
